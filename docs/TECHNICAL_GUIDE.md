...
上传先校验大小、扩展名、MIME 和文件签名，再进入私有存储与 OCR。OCR 失败不写入指标；全部字段逐项确认或修正后，报告才能进入 `confirmed`，候选行动排序才会读取它。
...
结果计算包含有效天数、缺失、异常值策略、组均值/中位数、差值和 Bootstrap 区间。数据不足时不输出方向性健康结论；AI 只解释服务端计算结果，输出需通过禁用诊断、调药、保证疗效等规则。
...
| 本地私有目录 | OBS 私有桶 | 接口边界完成；真实权限与生命周期待验收 |
