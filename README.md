...
- [M5 个体方案验收单](docs/V2_M5_ACCEPTANCE.md)
- [M6 执行反馈、复查和方案迭代验收单](docs/V2_M6_ACCEPTANCE.md)
- [M6 界面验收执行手册](docs/V2_M6_QA_RUNBOOK.md)
...
3. 配置 OBS、OCR 和 MaaS 的 HTTPS 端点、私有桶、模型名及受保护凭据，先使用合成或批准的脱敏数据逐项联调。
4. 将三项真实联调证据编号写入独立验收记录变量，执行 `scripts/cloud_preflight.py`；记录编号本身不替代日志和截图。
5. 使用根目录 `Dockerfile` 构建镜像，在 ECS 或容器服务运行。
6. 通过环境变量或密钥管理服务注入凭据，不要上传 `.env`。
