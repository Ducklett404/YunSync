...
- 任一项为“是”时不提供绕过按钮。
- 本阶段答案和放行状态仅保存在页面内存；刷新即清除。未完成或触发安全初筛时，直接访问其他主流程路由会被送回 `/start`。
...
当前 `/start` 是交互原型，不构成正式授权记录，也不会持久化安全问卷。现有前端路由守卫只用于验证交互闭环，不能代替服务端鉴权。正式实现必须由后端保存授权版本、时间、撤回状态和审计事件，并在所有报告与实验接口再次校验授权状态。
