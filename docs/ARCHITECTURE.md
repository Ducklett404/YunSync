...
6. 报告字节与可查询元数据分离：本地开发把合成文件保存到非公开目录，数据库只存随机对象键、摘要、大小和处理状态；华为 OBS 以 `reports/{user_id}/{random}` 私有键读写，配置、SDK 或服务异常时显式失败。
...
23. MaaS 只接收行动模板或服务端统计结果的白名单字段，要求 JSON 结构化返回；服务端再次执行长度、诊断/调药/注入模式、数值依据和安全边界校验，不合规时使用固定回退文案。
24. `cloud-preflight-v4` 分开报告适配器实现与真实云验收；只有 OBS、OCR、MaaS 三项脱敏联调均有外部记录编号时才通过真实提供方门禁，且不回显编号或密钥。
25. `/internal/metrics` 只使用方法、路由模板和状态类别等有界标签；令牌错误时拒绝访问，不记录对象 ID、查询参数或健康正文。
26. `m8-release-preflight-v1` 分开计算工程就绪与外部验收；不可变镜像、恢复、HTTPS、告警、专业、用户和客户证据缺一时不得宣称正式发布。
...
2. 行动模板治理和真实 MaaS 请求适配器已具备；首批模板仍需健康专业指导老师逐条复核，MaaS 仍需在已开通服务上完成脱敏红队验收。
